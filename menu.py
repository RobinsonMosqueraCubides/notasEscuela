import os
from estudiante import *
from notas import *
def menuNotas(select1, codigo):
    if select1 == 1:
    
        notaParcial = float(input("ingrese la nota del parcial\n"))
        agregarNotas(notaParcial,codigo,"parcial")
        menoSecundario()
    elif select1 == 2:
       
        quices = float(input("Ingrese la nota del quiz:\n"))
        agregarNotas(quices,codigo,"quiz")
        menoSecundario()
    elif select1 == 3:
           
        taller =  float(input("Ingrese la nota del taller:\n"))
        agregarNotas(taller,codigo,"taller")
        menoSecundario()
    else:
        print("Numero ingresado es invalido")
        menoSecundario()
    
        

def menoSecundario(select=2):
    if select == 1:

        print("Ingrese el codigo del estudiante")
        codigo = int(input(""))
        x = validarEstudiante(codigo)
        if x == False:
            print("Ingrese el Nombre del estudiante")
            nombre = input("")
            print("ingrese la edad del estudiante")
            edad = input("")
            alumno(codigo,nombre,edad)
            select2 = int(input("Desea agregar otro alumno\n1. Sí\n2. No\n"))
            if select2 == 1:
                menoSecundario(select2)
            elif select2 == 2:
                os.system("cls")
                menuPrincipal()
        else:
            print(f"Estudiante ya existe es: {student[codigo]['nombre']}")
            menuPrincipal()
           
    elif select == 2:
        print("ingrese el codigo del estudiante o 0 para regresar el menu principal:")
        codigo = int(input(""))
        if codigo != 0:
            x = validarEstudiante(codigo)
            if x == True:
                print("1. Notas de parcial\n2. Nota de quiz\n3. Nota de taller")
                select1 = int(input(""))
                menuNotas(select1,codigo)
            else:
                print("Estudiante no existe")
                menoSecundario()
        else:
            menuPrincipal()
    elif select == 3:
        id= int(input("ingrese el codigo del estudiante:\n"))
        x = validarEstudiante(id)
        if x == True:
            notasFinal = calculoNotas(id)
            print(f"|\t{student[id]['nombre']}\t|\t {student[id]['quiz']}\t|\t {student[id]['taller']}\t|\t {student[id]['parcial']}")
            print(f"Nota final {notasFinal}")
        else:
            menuPrincipal()
        menuPrincipal()
    elif select == 4:
        clave = pasarMateria()
        print(f"{student[clave]['nombre']}")
        menuPrincipal()
    else:
        menuPrincipal()

def menuPrincipal():
    
    while True:
        
        print("1. Registrar alumno\n2. Registrar notas\n3. Buscar Estudiante\n4. Listar estudiantes que no pasaron\n5. Salir")
        select = int(input(""))

        if select == 1:
            os.system("cls")
            menoSecundario(select)
            break
        elif select == 2:
            os.system("cls")
            menoSecundario(select)
            break
        elif select == 3:
            os.system("cls")
            menoSecundario(select)
            break
        elif select == 4:
            os.system("cls")
            menoSecundario(select)
        elif select == 5:            
            break        
        else:
            os.system("cls")
            print("Ingreso no valido, por favor ingrese un numero del 1 al 4")

menuPrincipal()

